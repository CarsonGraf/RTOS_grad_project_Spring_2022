...
int log_FRESULT(FRESULT fr, const char* message) {
    if(fr != FR_OK) 
        printf("File system error code %d at %s.\n", fr, message);
    return fr != FR_OK;
}
...
int eFile_Format(void){ // erase disk, add format
    ...
    //return log_FRESULT(f_mkfs("", , &buffer, _MAX_SS), "format");
    return 0;
}
...
int eFile_Mount(void){ // mount disk
	return log_FRESULT(f_mount(&g_sFatFs, "", 1), "mount");
}
...
int eFile_Create( const char name[]){  // create new file, make it empty 
    return log_FRESULT(f_open(&f, name, FA_CREATE_ALWAYS), "create");
}
...
int eFile_WOpen( const char name[]){      // open a file for writing 
	return log_FRESULT(f_open(&f, name, FA_WRITE), "wopen");
}
...
int eFile_Write( char data){
    ...
    int returnInt = log_FRESULT(f_write(&f, &data, 1, &successfulWrites), "write");
...
int eFile_WClose(void){ // close the file for writing
	return log_FRESULT(f_close(&f), "wclose"); 
}
...
int eFile_ROpen( const char name[]){      // open a file for reading 
	return log_FRESULT(f_open(&f, name, FA_READ), "ropen");  
}
...
int eFile_ReadNext( char *pt){       // get next byte 
    ...
    int returnInt = log_FRESULT(f_read(&f, pt, 1, &successfulReads), "read");
...
int eFile_RClose(void){ // close the file for writing
	return log_FRESULT(f_close(&f), "rclose");
}
...
int eFile_Delete( const char name[]){  // remove this file 
	return log_FRESULT(f_unlink(name), "delete");
}                             
...
int eFile_DOpen( const char name[]){ // open directory
    return log_FRESULT(f_opendir(&d, name), "dopen");
}
...
int eFile_DirNext(char** name, unsigned long *size){  // get next entry 
    int returnInt = log_FRESULT(f_readdir(&d, &fi), "dirnext");
...
int eFile_DClose(void){ // close the directory
	return log_FRESULT(f_closedir(&d), "dclose");
}
...
int eFile_Unmount(void){ 
	return log_FRESULT(f_mount(0, "", 1), "unmount"); 
}
