...
//-----------------------ST7735_OutDec-----------------------
// Literally just OutUDec but signed
void ST7735_OutDec(int32_t num){
  Messageindex = 0;
  int negative = 0;
  uint32_t n;
  if(num < 0) {
...
//------------ST7735_Message_Text_Only------------
// String draw and number output.  
// Input: device  0 is on top, 1 is on bottom
//        line    row from top, 0 to 7 for each device
//        pt      pointer to a null terminated string to be printed
void ST7735_Message_Text_Only(uint32_t  d, uint32_t  l, char *pt){
    uint32_t column = 0;
    uint32_t row = d ? 0 : 8;
    row += l;
